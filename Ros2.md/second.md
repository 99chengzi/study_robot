...
1. **先看 `loopControl()`**（第 310 行附近）——这是总入口，理解整个流程的骨架
2. **再看 `normalGame()`**（第 380 行附近）——比赛中的核心决策逻辑
3. **然后看 `positioning()`**（第 590 行附近）——理解阵型站位
4. **接着看 `setEthercatCommond()`**（第 760 行附近）——理解命令怎么发出去的
5. **最后看构造函数**（文件开头）——理解机器人怎么"出生"和连接各个模块

**读代码时心里记住几件事：**
- AgentID = "我是几号机器人"，决定了我的角色和责任
- 所有决策都基于 `world_model_info_`（共享世界模型）
- 15ms 一个循环，每个循环都会重新决策，所以"反应快"
- 代码中的中文注释（如 `/// @hbx`）是不同开发者留下的标识
